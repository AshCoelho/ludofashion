...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Edita o usuario no banco de dados chamando o método editar da classe Usuario
    $usuario->editar($id, $email, $senha, $cpf, $nome_usuario, $nascimento);
    // Redireciona para a página inicial após a edição
    header('Location: index.php');
} else {
    // Obtém o ID do usuario a ser editado a partir dos parâmetros da URL
    $id = $_GET['id'];
    // Obtém os dados do usuario a ser editado usando o método listar da classe Usuario
    $usuarioEditar = $usuario->listar()[array_search($id, array_column($usuario->listar(), 'id'))];
}
?>
